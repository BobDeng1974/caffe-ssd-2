...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->layer_param_.propagate_down_size() == 0) {
    this->layer_param_.add_propagate_down(true); // mbox_loc 层
    this->layer_param_.add_propagate_down(true); // mbox_conf 层
    this->layer_param_.add_propagate_down(false); //mbox_priorbox 层
    this->layer_param_.add_propagate_down(false); // label 层
  }
 /* 在这里定义的 message 参数，在 caffe.proto 定义的
...
*/
  ...
  num_ = bottom[0]->num(); // bottom[0] 是 mbox_loc 层
  num_priors_ = bottom[2]->height() / 4; // bottom[2] 是 mbox_priorbox 层
  ...
  vector<int> loss_shape(1, 1); //初始化了1个值为1的元素
  ...
  // 分类损失函数类型：softmax 和 SigmoidCrossEntropyLoss
